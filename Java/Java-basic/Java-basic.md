...
Java DataBase Connectivity，Java 数据库连接，用 Java 语言操纵数据库。

**JDBC 本质**

- 其实就是官方（sun 公司）定义的一套操作所有关系型数据库的规则，即接口；
- 各个数据库厂商去实现这套接口，提供数据库驱动 jar 包；
- 我们可以使用这套接口（JDBC）编程，真正执行的代码是驱动 jar 包中的实现类对应的实现方法。

## 10.1、快速入门

1. 导入驱动 jar 包；
   - 复制 `mysql-connection-5.1.37(26)-bin.jar` 到创建好的 libs 项目目录下面；
   - 右键 libs -> Add As library 真正的把 jar 包加载进来；
2. 注册驱动
...
- `int executeUpdate(String sql)`：执行DML（insert、update、delete）语句、DDL（create、alter、drop）语句，返回值表示影响的行数，可以通过这个影响的行数判断 DML 语句是否执行成功：`返回值 > 0` 则执行成功，反之则失败。
- `ResultSet executeQuery(String sql)`：执行DQL（select）语句，返回值为结果集对象。
